...
@interface SelectRangeViewController ()<UIGestureRecognizerDelegate>{
...
- (void) initializeControls {
    ...
    _rightLimit = self.totalPlot.frame.size.width - MARGIN_VAL - frame2.size.width / 2;
    self.rightViewLeadingConstraint.constant = self.totalPlot.frame.size.width - 20 - frame2.size.width / 2;
...
- (void) touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (dragging) {
        ...
        if ([window isEqual:_viewLeft]) {
            ...
            float offsetX = self.scrollView.contentOffset.x + (touchLocation.x - oldX) * (double)self.totalWidthConst.constant / (double)self.totalPlot.frame.size.width;
            if (offsetX < 0)
            {
                offsetX = 0;
            }
...
- (void) adjustMinMaxFrames {
    ...
    self.displayRangeLeadingConst.constant = minFrame * (double)self.totalPlot.frame.size.width / (double)self.totalfile.totalFrames;
    self.displayRangeWidthConst.constant = (maxFrame - minFrame) * (double)self.totalPlot.frame.size.width / (double)self.totalfile.totalFrames;
}
...
- (IBAction)onNext:(id)sender {
    SaveViewController * saveViewer = [self.storyboard instantiateViewControllerWithIdentifier:@"SaveViewController"];
...
- (IBAction)displayDragEnter:(id)sender {
    
}
- (IBAction)displayDragExit:(id)sender {
}
@end
